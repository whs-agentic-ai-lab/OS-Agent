  ...
  useEffect(() => {
    ...
    Promise.all([getHarnessStatus(), getFixtureHarnessStatus()])
      .then(([nextLiveStatus, nextFixtureStatus]) => {
        ...
        setFixtureStatus(nextFixtureStatus);
        setError(null);
      })
      .catch((reason) => {
        if (!isActive) return;
  ...
  return (
    <section className="harness-panel" aria-labelledby="harness-title">
      ...
      <div className="harness-component-grid" aria-label="Harness 구성요소 연결 상태">
        {(liveStatus?.components ?? []).map((component) => (
        ...
        {isLoading ? <p className="loading-message">Adapter 상태 확인 중…</p> : null}
      </div>
                  ...
                  return (
                    <li key={action.candidate.candidate_id}>
                      <div>
                        ...
                        <strong>{action.candidate.tool_name}</strong>
