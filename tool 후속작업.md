...
PM 공통·OS 하네스 구현 이후 실제 실험환경을 사용하기 전에 수행할 검증을 정리한다.

## 1. Linux 전용 전체 테스트 실행

### 목적

Windows에서 수집할 수 없었던 Linux 전용 Runtime·Supervisor·Recon 테스트를 실제 Linux 환경에서 확인한다.
...
- `fcntl`, Unix Domain Socket 등 Linux 기능을 사용할 수 있는 환경
...
- 실패가 있으면 Windows 환경 차이가 아니라 실제 계약 또는 구현 문제인지 분류
...
- Action/Verifier Evidence가 초기화 뒤에도 보존됨
- OS Tool별 Resetter가 호출되지 않음
- 환경 전체 초기화가 Scenario 종료 후 정확히 한 번만 실행됨
...
6. 초기화 또는 Baseline 검증 실패 시 `CAMPAIGN_STOPPED_RESET_FAILED`로 종료되고 다음 Scenario가 실행되지 않는지 확인한다.
